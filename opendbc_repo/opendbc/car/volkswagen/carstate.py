...
#SP:def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
  def update(self, can_parsers) -> structs.CarState:
    ...
    self.frame += 1
    #原SP：return ret, ret_sp
    return ret

#原SP:def update_pq(self, pt_cp, cam_cp, ext_cp) -> tuple[structs.CarState, structs.CarStateSP]:
  def update_pq(self, pt_cp, cam_cp, ext_cp) -> structs.CarState:
    ret = structs.CarState()
#SP ADD:ret_sp = structs.CarStateSP()
...
#ADD MLB END
  ...
  @staticmethod
  #原SP：def get_can_parsers(CP, CP_SP):
  def get_can_parsers(CP):
    if CP.flags & VolkswagenFlags.PQ:
      return CarState.get_can_parsers_pq(CP)
    # another case of the 1-50Hz,分析下方语句为函数初始化，由于用到pt_messages，故而需要修改增加。
